...
var (
	...
	KV_TTL = "select lastTIme from rstore_kv  where  \"rkey\" = $1"
	...
	HASH_TTL = "select max(lastTIme) from rstore_hash  where  \"rkey\" = $1"
	...
	ZSET_TTL = "select max(lastTIme) from rstore_zset  where  \"rkey\" = $1"
	...
	SET_TTL = "select max(lastTIme) from rstore_set  where  \"rkey\" = $1"
)
...
//why return 1 ? because it is the key number not the record number
func (pg *Postgres) DEL(key string) (int, error) {
